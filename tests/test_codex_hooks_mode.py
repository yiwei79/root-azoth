...
def _prepare_repo(root: Path) -> None:
    ...
    (adapter / "hooks.json.template").write_text('{"hooks": {"UserPromptSubmit": []}}\n', encoding="utf-8")
...
def test_codex_hooks_mode_set_verbose_and_restore_calm(tmp_path: Path) -> None:
    ...
    assert (tmp_path / ".codex" / "hooks.mode.local").read_text(encoding="utf-8").strip() == "verbose"
    assert (
        tmp_path / ".codex" / "hooks.json"
    ).read_text(encoding="utf-8") == (
        tmp_path / "kernel" / "templates" / "platform-adapters" / "codex" / "hooks.verbose.json.template"
    ).read_text(encoding="utf-8")
...
def test_codex_hooks_mode_status_reports_unsynced_file(tmp_path: Path) -> None:
    _prepare_repo(tmp_path)
    (tmp_path / ".codex" / "hooks.json").write_text('{"hooks": {"Unexpected": []}}\n', encoding="utf-8")
