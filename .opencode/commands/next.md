...
10b. **Acquire write claim**: After writing `scope-gate.json`, acquire the write claim so
    ...
    ```json
    {
      ...
      "governance_mode": "<governed | standard — normalized from backlog primary delivery_pipeline and target_layer>",
      "pipeline_command": "<optional: auto | dynamic-full-auto | deliver | deliver-full — only if the human explicitly selected the delivery command in the same approval>",
      "delivery_pipeline": "<governed | standard — bridge-only legacy mirror of governance_mode when older consumers still require it>",
      "target_layer": "<M1 | infrastructure | … — from backlog primary target_layer>"
    }
    ```

    Persist `governance_mode` on every approved scope. Persist `pipeline_command`
    only when the human explicitly selected the delivery command in the same turn;
    otherwise leave it unset until the chosen delivery command opens or updates
    ...
    **Governed delivery (mechanical):** If the normalized scope records
    `governance_mode: governed` (or an older bridge gate still implies governed via
    `delivery_pipeline: governed` / `target_layer: M1`), the PreToolUse hook **blocks Write/Edit** until
    ...
    Confirm: "scope-gate.json written — intent declared with governance_mode recorded; select a delivery pipeline next. If none was specified, use /auto and run Stage 0 before implementation. Governed scopes still require pipeline-gate.json after Stage 0 of the chosen delivery pipeline."
