...
| 4        | `knowledge == instruction-refinement AND complexity == simple AND risk == cosmetic` | `[planner, builder, architect]` | Lightweight instruction-refinement lane — only for low-risk cosmetic refinements |
| 5        | `knowledge == instruction-refinement`       | `[architect, reviewer, planner, evaluator, builder, architect]` | Full pipeline fallback — ambiguous, additive, medium+, governed, or otherwise high-stakes refinement; inject l2-evidence-review into architect |
...
**Rule 4 (lightweight instruction-refinement)** — Use the lightweight lane only
when `knowledge == instruction-refinement AND complexity == simple AND risk == cosmetic`.
This is the exact qualifier for low-risk instruction tuning: minor wording,
clarity, or formatting changes to an existing instruction surface with no new
behavioral surface area. Because the change is both simple and cosmetic, the
pipeline can skip reviewer and evaluator while still preserving planner scoping
and architect closeout.

**Rule 5 (instruction-refinement fallback)** — Any instruction-refinement goal
that does not match the exact lightweight qualifier falls back to the full
pipeline. This includes additive refinements, medium or complex refinements,
ambiguous risk, governance-touching refinements, kernel-adjacent refinements,
and any case where the safe classification is unclear. The fallback rule is
kept directly below the lightweight rule so first-match routing stays
conservative: governed and kernel rules still win above it, and all remaining
instruction-refinement work receives reviewer validation and evaluator quality
gates. An l2-evidence-review phase is injected into the architect stage so
...
is produced for the fallback path:
