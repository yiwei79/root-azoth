#!/usr/bin/env python3
"""Build a no-write review packet for manifest-approved personal knowledge cards."""
...
from personal_knowledge_recall import (
...
def build_review_packet(
    ...
    *,
    ...
    results = recall_cards(personal_root, allowed_use=allowed_use, as_of=as_of)
...
def _review_reason(status: str) -> str:
    return {
        "current": "card freshness window is current",
        "review_due": "card review_after date has passed",
        "stale_for_use": "card is not active and should not be treated as current context",
    }.get(status, "card freshness metadata is missing or not actionable")


def _next_safe_action(due_cards: list[dict[str, Any]]) -> str:
    if not due_cards:
        return "continue harness use; no personal knowledge review is due"
    ids = ", ".join(card["card_id"] for card in due_cards)
    return f"review source refs for {ids} through an approved personal-knowledge review lane"
...
def main(argv: list[str] | None = None) -> int:
    ...
    return 2 if args.fail_on_due and packet["summary"]["requires_operator_review"] else 0
