...
posture:
  always_do:
    ...
    - Consult M3 episodes before goal classification when episodes.jsonl exists
    - Track token budget across pipeline stages and compress when approaching ceiling
    - Apply model_tier to every spawn contract
  ask_first:
    - Scope expansion beyond original goal
    - Pipeline composition changes
    - Skipping any declared human gate
    - Mid-pipeline re-scope when deviation detected
  never_auto: []
pipeline_stages:
  - goal-clarification
  - orchestration
...
When the context budget exceeds 80%, compress `prior_stage_summaries` to:
```yaml
compressed: true
...
On Codex, the default execution budget is `10` threads at depth `2`. Only `orchestrator`, `research-orchestrator`, and `architect` may spend depth > 1, and only when the spawn carries `execution_budget`; all other agents are leaf-only. Reserve child capacity for queen agents instead of saturating every slot, and prefer flat fan-out when nesting does not add value.
